...
window.operateEvents = {
    ...
    'click .btn_pointhospital': function (e, value, row, index) {
        e.stopPropagation();
        var tr = $(this).closest("tr");
        onPointHospital(row["Id"], tr);
    },
    'click .btn_setrange': function (e, value, row, index) {
        e.stopPropagation();
        var tr = $(this).closest("tr");
        onSetRange(row["Id"], row["RangeTypeId"], row["Name"], tr);
    }
};
...
// 执行事件
{
    ...
    function onSetRange(id, rangetypeid, name) {
        window.localStorage.title = name;
        window.location.href = "/exp/setRange?type=1&labid=" + id + "&rangetypeid=" + rangetypeid;
    }
}
...
// 表格数据绑定格式化
{
    function actionFormatter01() {
        return [
            ...
            '<i class="glyphicon glyphicon-home btn_pointhospital" data-toggle="tooltip" data-placement="bottom" title="指定研究中心"></i>',
